# VUC — Dual License Agreement

## License 1: Freemium (Free — Personal, Non-Commercial Use Only)

Copyright (c) 2026 Fractal Resonance Grand. All rights reserved.

Permission is granted, free of charge, to any person obtaining this software to use it for **personal, non-commercial purposes only**, subject to the following limitations:

- **100MB maximum file size** per compression operation
- **Basic backends only** (VRLE, VLZR, Passthrough)
- **No batch processing**
- **No redistribution, sublicensing, or resale**
- **No modification, reverse engineering, or decompilation**

THE SOFTWARE IS PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND.

---

## License 2: Commercial (Paid — Everything Else)

All other uses require a paid commercial license. This includes but is not limited to:

- Business, corporate, government, or institutional use
- Any use generating revenue (direct or indirect)
- File sizes above 100MB
- Advanced backends (VLZX, VLZH, DEFLATE, LZMA, CRM1, CRMW, VC1H)
- Batch or automated processing
- Redistribution, embedding, or OEM integration
- Modification, customization, or derivative works

**Email for pricing and licensing:** license@fractalresonance.grand

Enterprise licenses include:
- Unlimited file sizes
- All compression backends
- Batch processing
- SLA support
- Custom integration options
- Hardware-bound license keys
- Volume discounts (100+ seats)

*Fractal Resonance Grand — 2026*